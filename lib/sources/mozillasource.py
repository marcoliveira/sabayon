...
def write_pref_file(pref_full_path, prefs):
    dprint("writing pref file (%s)", pref_full_path)
...
''' % (config.PACKAGE, config.VERSION))
    ...
    prefs.sort(lambda a, b: cmp(a.get_key(), b.get_key()))
    for change in prefs:
...
class MozillaDelegate (userprofile.SourceDelegate):
    ...
    def commit_change (self, change, mandatory = False):
        ...
        if mandatory:
            self.committed_mandatory_prefs.append(change)
        else:
            self.committed_prefs.append(change)
    ...
    def sync_changes (self):
        ...
        if ini:
            ...
            # XXX - jrd, should we be adding prefs.js?
            pref_rel_path = os.path.join(pref_rel_dir, "prefs.js")
            ...
            if len(self.committed_prefs) > 0:
                ...
                pref_full_path = self.get_full_path(pref_rel_path)
                write_pref_file(pref_full_path, self.committed_prefs)
                self.source.storage.add (pref_rel_path,
                                         self.home_dir,
                                         self.name,
                                         {"is_pref_file" : True,
                                          "mandatory"    : False})
    ...
    def __apply_foreach (self, source, path):
        ...
        is_pref_file = attributes.get("mandatory", False)

        dprint("apply_foreach  %s %s %s", source, self.home_dir, path)
        if is_pref_file:
            ...
            target_pref_rel_path = os.path.join(os.path.dirname(path), "prefs.js")
            target_pref_full_path = self.get_full_path(target_pref_rel_path)
            if os.path.isfile(target_pref_full_path):
